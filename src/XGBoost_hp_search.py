"""
风险等级多分类建模脚本：使用 XGBoost 进行超参数搜索与最终模型训练。

核心思想：
1. 从 data/Dataset.csv 中读取风险样本数据，将前 23 列作为特征，最后 1 列作为风险标签。
2. 对标签进行整数编码（如 无风险→0、低风险→1、...、极高风险→4），并对特征做标准化预处理：
   - 二元变量用“是/否”映射为 0/1；
   - 有序类别变量按顺序映射为整数；
   - 无序类别变量使用 One-Hot 编码。
3. 先把数据切分为训练集、验证集、测试集，使用 Optuna 自动搜索 XGBoost 的最优超参数。
4. 搜索得到最佳参数后，使用训练集+验证集重新训练最终模型，并在测试集上评估分类效果。
5. 输出准确率、分类报告、混淆矩阵，并保存最优模型与搜索结果日志。

这段代码的本质是：自动化寻找一个最能区分不同风险等级的 XGBoost 模型，并验证其泛化能力。
"""
